...
def main() -> int:
    ...
    parser.add_argument("--member-format", choices=("integer", "uuid-page-id"),
                        default="integer")
    args = parser.parse_args()
    if args.member_format == "uuid-page-id" and args.tie_order != "numeric":
        parser.error("uuid-page-id normalization requires numeric tie order")
    ...
    while True:
        ...
        try:
            if args.member_format == "uuid-page-id":
                text = member.decode("ascii")
                if len(text) != 36 or any(text[i] != "-" for i in (8, 13, 18, 23)):
                    raise ValueError("invalid UUID page ID")
                compact = text.replace("-", "")
                if len(compact) != 32 or any(c not in "0123456789abcdef" for c in compact):
                    raise ValueError("invalid canonical UUID page ID")
                member_number = int(compact, 16)
                if member_number > 0x7fffffff:
                    raise ValueError("UUID page ID exceeds INT32")
                member = str(member_number).encode("ascii")
            else:
    ...
    result = {
        "member_format": args.member_format,
        "count": count,
