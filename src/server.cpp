...
struct ReplyBoundary {
  ...
  std::uint64_t required_kafka_offset{0};
};
...
struct EndpointSession : detail::PubSubSession {
  ...
  void require_kafka_ack(std::uint64_t first_sequence,
                         std::uint64_t logical_offset) noexcept {
    bool found = false;
    for (auto& reply : replies) {
      if (reply.sequence < first_sequence) continue;
      reply.required_kafka_offset =
          std::max(reply.required_kafka_offset, logical_offset);
      found = true;
    }
    if (!found) {
      next_reply_kafka_offset =
          std::max(next_reply_kafka_offset, logical_offset);
    }
  }

  void release_kafka_ack(std::uint64_t logical_offset) noexcept {
    for (auto& reply : replies) {
      if (reply.required_kafka_offset != 0 &&
          reply.required_kafka_offset <= logical_offset) {
        reply.required_kafka_offset = 0;
      }
    }
    if (next_reply_kafka_offset != 0 &&
        next_reply_kafka_offset <= logical_offset) {
      next_reply_kafka_offset = 0;
    }
  }
...
class ReplicationRuntime {
 public:
...
#ifdef GOBLIN_HAS_KAFKA
  ReplicationRuntime(KafkaJournal* journal, KafkaAckMode ack_mode,
                     std::size_t buffer_bytes, std::size_t pending_limit_bytes,
                     std::uint64_t initial_offset)
      : buffer_bytes_(buffer_bytes),
        released_offset_(initial_offset),
        pending_limit_bytes_(pending_limit_bytes),
        journal_(journal),
        broker_ack_(journal != nullptr && ack_mode == KafkaAckMode::broker) {}
#else
  ReplicationRuntime(std::size_t buffer_bytes, std::uint64_t initial_offset)
      : buffer_bytes_(buffer_bytes), released_offset_(initial_offset) {}
#endif

  class Scope {
   public:
    Scope(ReplicationRuntime& runtime, Store& store,
          EndpointSession* session = nullptr) noexcept
        : runtime_(runtime),
          store_(store),
          session_(session),
          first_sequence_(session != nullptr ? session->next_output_sequence
                                             : 0),
          pending_at_begin_(runtime.pending_.size()) {
      runtime_.begin();
    }
    ~Scope() noexcept {
      runtime_.finish(store_, session_, first_sequence_, pending_at_begin_);
    }
   ...
   private:
    ...
    Store& store_;
    EndpointSession* session_{nullptr};
    std::uint64_t first_sequence_{0};
    std::size_t pending_at_begin_{0};
  };
  ...
  [[nodiscard]] bool subscribe(EndpointSession& session, Store& store,
    ...
    auto hello_state = store.replication_state();
    if (broker_ack_) hello_state.offset = released_offset_;
    const auto hello = encode_firehose_hello(hello_state);
  ...
  void remove(EndpointSession& session) noexcept {
    ...
    for (auto& batch : awaiting_ack_) {
      std::erase_if(batch.waiters, [&session](const ReplyWaiter& waiter) {
        return waiter.session == &session;
      });
    }
  }
  ...
  void finish(Store& store, EndpointSession* session,
              std::uint64_t first_sequence,
              std::size_t pending_at_begin) noexcept {
    if (batch_depth_ == 0) return;
    if (session != nullptr && pending_.size() > pending_at_begin) {
      try {
        const auto duplicate = std::any_of(
            batch_waiters_.begin(), batch_waiters_.end(),
            [session, first_sequence](const ReplyWaiter& waiter) {
              return waiter.session == session &&
                     waiter.first_sequence == first_sequence;
            });
        if (!duplicate) {
          batch_waiters_.push_back(
              ReplyWaiter{.session = session,
                          .first_sequence = first_sequence});
        }
      } catch (const std::exception& exception) {
        if (broker_ack_ && session != nullptr) {
          session->require_kafka_ack(
              first_sequence, std::numeric_limits<std::uint64_t>::max());
        }
        fail(exception.what());
      } catch (...) {
        if (broker_ack_ && session != nullptr) {
        ...
        fail("unknown broker acknowledgement bookkeeping failure");
      }
    }
    if (--batch_depth_ != 0) return;
    if (pending_.empty() || failed_) {
      if (failed_ && broker_ack_ && session != nullptr) {
        session->require_kafka_ack(
            first_sequence, std::numeric_limits<std::uint64_t>::max());
      }
  ...
  [[nodiscard]] bool input_backpressured() const noexcept {
    return broker_ack_ && pending_limit_bytes_ != 0 &&
           awaiting_ack_bytes_ >= pending_limit_bytes_;
  }

  static void append_info(void* context, std::string& out) {
    out += "# Kafka\r\n";
#ifdef GOBLIN_HAS_KAFKA
    const auto& self = *static_cast<ReplicationRuntime*>(context);
    if (self.journal_ != nullptr) {
      const auto stats = self.journal_->stats();
      out += "kafka_journal_enabled:1\r\n";
      out += std::string("kafka_ack_mode:") +
             (self.broker_ack_ ? "broker\r\n" : "queued\r\n");
      out += "kafka_acknowledged_logical_offset:" +
             std::to_string(stats.acknowledged_logical_offset) + "\r\n";
      out += "kafka_pending_records:" +
             std::to_string(stats.pending_records) + "\r\n";
      out += "kafka_pending_bytes:" + std::to_string(stats.pending_bytes) +
             "\r\n";
      out += "kafka_oldest_pending_age_ms:" +
             std::to_string(stats.oldest_pending_age_ms) + "\r\n";
      out += "kafka_retained_batch_bytes:" +
             std::to_string(self.awaiting_ack_bytes_) + "\r\n";
      out += std::string("kafka_input_backpressured:") +
             (self.input_backpressured() ? "1\r\n" : "0\r\n");
      return;
    }
#else
    (void)context;
#endif
    out += "kafka_journal_enabled:0\r\n";
    out += "kafka_ack_mode:off\r\n";
  }

#ifdef GOBLIN_HAS_KAFKA
  void release_acknowledged() noexcept {
    if (!broker_ack_ || journal_ == nullptr) return;
    const auto acknowledged = journal_->stats().acknowledged_logical_offset;
    while (!awaiting_ack_.empty() &&
           awaiting_ack_.front().last_offset <= acknowledged) {
      auto batch = std::move(awaiting_ack_.front());
      awaiting_ack_.pop_front();
      awaiting_ack_bytes_ = batch.bytes <= awaiting_ack_bytes_
                                ? awaiting_ack_bytes_ - batch.bytes
                                : 0;
      for (const auto& waiter : batch.waiters) {
        if (waiter.session != nullptr) {
          waiter.session->release_kafka_ack(batch.last_offset);
        }
      }
      released_offset_ = batch.last_offset;
 ...
 private:
  ...
  struct AwaitingAckBatch {
    std::uint64_t last_offset{0};
    std::size_t bytes{0};
    ReplicationBatch batch;
    std::vector<ReplyWaiter> waiters;
  };
  ...
  void commit(Store& store) noexcept {
    ...
    try {
      ...
      const auto last_offset =
          state.offset + static_cast<std::uint64_t>(batch.mutations.size());
      if (broker_ack_) {
        for (const auto& waiter : batch_waiters_) {
          if (waiter.session != nullptr) {
            waiter.session->require_kafka_ack(waiter.first_sequence,
                                              last_offset);
          }
        }
      }
      ...
      if (broker_ack_) {
        ...
        awaiting_ack_.push_back(AwaitingAckBatch{
            .last_offset = offset,
            .bytes = bytes,
            .batch = std::move(batch),
            .waiters = std::move(batch_waiters_)});
  ...
  std::vector<ReplyWaiter> batch_waiters_;
  std::deque<AwaitingAckBatch> awaiting_ack_;
  std::size_t batch_depth_{0};
  std::size_t buffer_bytes_{0};
  std::uint64_t released_offset_{0};
  std::size_t pending_limit_bytes_{0};
  std::size_t awaiting_ack_bytes_{0};
#ifdef GOBLIN_HAS_KAFKA
  KafkaJournal* journal_{nullptr};
#endif
  bool broker_ack_{false};
  bool failed_{false};
 ...
 public:
  ...
  void serve_ready(Store& store, ReplicationRuntime& replication) {
    ...
    while (index < waiters_.size()) {
      ...
      ReplicationRuntime::Scope replication_scope(replication, store, &session);
      if (!parsed.ok()) {
...
[[nodiscard]] bool has_writable_output(EndpointSession& session) noexcept;
...
[[nodiscard]] short client_poll_events(Client& client,
                                       bool input_paused) noexcept {
#ifdef GOBLIN_HAS_TLS
...
#endif
  short events = 0;
  if (!input_paused && !client.firehose && !client.dump_world &&
...
[[nodiscard]] bool has_ready_regular_output(
    const EndpointSession& session) noexcept {
  return has_pending_regular_output(session) &&
         session.replies[session.reply_index].required_kafka_offset == 0;
}
...
[[nodiscard]] bool has_writable_output(EndpointSession& session) noexcept {
  ...
  const bool has_push = peek_unsolicited(session, push);
  if (!has_pending_regular_output(session)) return has_push;
  if (has_push &&
      push.sequence < session.replies[session.reply_index].sequence) {
    return true;
  }
  return has_ready_regular_output(session);
}
...
// One process-wide streamed snapshot. The child writes the native GCSN image to
// a blocking pipe while the parent forwards one bounded RESP3 streamed-string
// chunk at a time. Keeping the pipe and session association here lets every
// transport share the same serializer without putting a database-sized object
// in a client output buffer.
class DumpWorldRuntime {
 public:
  ~DumpWorldRuntime() {
    if (read_fd_ >= 0) (void)::close(read_fd_);
  }

  DumpWorldRuntime(const DumpWorldRuntime&) = delete;
  DumpWorldRuntime& operator=(const DumpWorldRuntime&) = delete;
  DumpWorldRuntime() = default;

  [[nodiscard]] bool start(EndpointSession& session, Store& store,
                           bool with_accelerator, std::string& error) {
    const auto started = store.start_background_dump(with_accelerator);
    switch (started.status) {
      case Store::DumpStartStatus::Started:
        break;
      case Store::DumpStartStatus::AlreadyRunning:
        error = "ERR background snapshot already in progress";
        return false;
      case Store::DumpStartStatus::PipeFailed:
        error = "ERR cannot create background snapshot pipe";
        return false;
      case Store::DumpStartStatus::ForkFailed:
        error = "ERR cannot fork for background snapshot";
        return false;
      case Store::DumpStartStatus::HugeTlbUnsafe:
    ...
    session_ = &session;
    read_fd_ = started.read_fd;
    bytes_ = 0;
    pipe_eof_ = false;
    read_failed_ = false;
    orphaned_ = false;
    session.dump_world = true;
  ...
  [[nodiscard]] int notification_fd() const noexcept {
    return read_fd_ >= 0 && session_ != nullptr &&
                   !has_pending_output(*session_)
               ? read_fd_
               : -1;
  }
  [[nodiscard]] bool needs_reap() const noexcept {
    return pipe_eof_ || orphaned_;
  }
  [[nodiscard]] bool active_for(const EndpointSession& session) const noexcept {
    return session_ == &session;
  }

  [[nodiscard]] bool pump(Store& store) {
    if (session_ == nullptr) {
      if (!store.background_dump_in_progress()) return false;
      if (!store.reap_background_dump()) return false;
      orphaned_ = false;
      pipe_eof_ = false;
      return true;
    }
    if (has_pending_output(*session_)) return false;

    if (!pipe_eof_) {
      static thread_local std::array<char, 64U * 1024U> buffer;
      const auto received = ::read(read_fd_, buffer.data(), buffer.size());
      if (received > 0) {
        const auto size = static_cast<std::size_t>(received);
        const std::size_t prior_size = session_->output.size();
        session_->output.push_back(';');
        session_->output.append(std::to_string(size));
        session_->output.append("\r\n");
        session_->output.append(buffer.data(), size);
        session_->output.append("\r\n");
        bytes_ += size;
        session_->record_reply(prior_size);
        return true;
      }
      if (received < 0 && errno == EINTR) return true;
      if (received < 0 && would_block()) return false;
      if (received < 0) read_failed_ = true;
      (void)::close(read_fd_);
      read_fd_ = -1;
      pipe_eof_ = true;
    }

    const auto outcome = store.reap_background_dump();
    if (!outcome) return false;
    EndpointSession* completed = session_;
    session_ = nullptr;
    pipe_eof_ = false;
    if (*outcome && !read_failed_) {
      const std::size_t prior_size = completed->output.size();
      completed->output.append(";0\r\n");
      completed->record_reply(prior_size);
      completed->quit_after_write = true;
      std::cout << "goblin-core: streamed " << bytes_
                << " snapshot bytes\n" << std::flush;
    } else {
      // A streamed blob has no legal in-band error after its first chunk. Close
      // without the terminator so the receiver must reject the partial image.
      completed->close_requested = true;
      std::cerr << "goblin-core: streamed snapshot FAILED after " << bytes_
                << " bytes\n";
    }
    return true;
  }
...
void dispatch_resp_command(EndpointSession& session, Store& store,
  ...
  } else if (command.type == CommandType::goblin_dumpworld) {
    if (session.resp_version != resp::Version::resp3) {
      resp::append_error(session.output,
                         "ERR GOBLIN.DUMPWORLD requires HELLO 3");
    } else {
      bool with_accelerator = true;
      bool valid = true;
      if (!command.args.empty()) {
        if (equals_ascii_ci(command.args.front(), "NOACCEL")) {
          with_accelerator = false;
        } else if (!equals_ascii_ci(command.args.front(), "ACCEL")) {
          valid = false;
          resp::append_error(
              session.output,
      ...
      if (valid) {
        ...
        blocking_lists.remove(session);
        std::string error;
        if (!dump_world.start(session, store, with_accelerator, error)) {
          resp::append_error(session.output, error);
        }
      }
    }
...
[[nodiscard]] bool process_buffered_commands(Client& client,
                                             ...
                                             const NestedCommandDispatch&
                                                 nested_dispatch) {
  if (client.blocked_list || client.firehose || client.dump_world ||
      replication.input_backpressured()) {
    return true;
  }
  ...
  const CommandExecutionOptions exec_options{
      ...
      .append_info = &ReplicationRuntime::append_info,
  ...
  while (!client.read_backpressured && !client.blocked_list &&
    ...
    dispatch_resp_command(client, store, pubsub, watches, blocking_lists,
                          replication, dump_world, client.fields, exec_options);
    if (client.close_after_write || client.firehose || client.dump_world) {
      break;
    }
    compact_output_if_needed(client);
    update_read_backpressure(client, config);
  ...
  if (client.firehose || client.dump_world) {
    client.parser.clear();
    client.inbuf.clear();
  }
...
[[nodiscard]] bool read_client(Client& client, Store& store,
  ...
  for (;;) {
    if (client.read_backpressured || replication.input_backpressured()) {
      return true;
    }
    ...
    if (received.status == TransportIoStatus::Progress && received.bytes > 0) {
      ...
      if (!client.blocked_list) {
        if (!process_buffered_commands(client, store, pubsub, watches,
                                       ...
                                       auth_database, replication, dump_world,
                                       nested_dispatch)) {
          return false;
        }
      }
  ...
  while (has_writable_output(client)) {
    ...
    if (write_push) {
      ...
      while (last_reply + 1 < client.replies.size() &&
             client.replies[last_reply + 1].required_kafka_offset == 0 &&
             (!has_push ||
...
[[nodiscard]] bool run_polled_targets(
  ...
  const auto process_polled_endpoint = [&]<class Consumer, class Producer>(
                                           ...
                                           Consumer& consumer,
                                           Producer& producer) -> bool {
    bool output_progress = flush_polled_output(ep, producer);
    if (dump_world.active_for(ep) && !has_pending_output(ep)) {
      output_progress = dump_world.pump(store) || output_progress;
      output_progress = flush_polled_output(ep, producer) || output_progress;
    }
    ...
    if (replication.input_backpressured()) {
      return output_progress;
    }
    ...
    if (ep.wire_mode == detail::WireMode::resp2 ||
      ...
      while (!ep.blocked_list && ep.parser.pop_into(ep.fields)) {
        dispatch_resp_command(ep, store, pubsub, watches, blocking_lists,
                              replication, dump_world, ep.fields, exec_options);
        if (ep.quit_after_write || ep.blocked_list || ep.firehose ||
            ep.dump_world) {
          break;
        }
      }
      if (ep.firehose || ep.dump_world) {
        ep.parser.clear();
        ep.inbuf.clear();
      }
  ...
  const auto process_exasock_target = [&](ExasockRuntimeTarget& target) -> bool {
    ...
    for (std::size_t offset = 0; offset < count; ++offset) {
      ...
      if (keep && has_writable_output(client)) {
        const bool before_pending = has_writable_output(client);
      ...
      if (keep && !client.read_backpressured &&
          !replication.input_backpressured() && has_buffered_work(client)) {
        keep = process_buffered_commands(
            ...
            auth_database, replication, dump_world, nested_dispatch);
        did_work = true;
      }
      ...
      if (keep && !replication.input_backpressured() && !client.firehose &&
          !client.dump_world &&
          !client.read_backpressured) {
        pfd.events |= POLLIN;
      }
      ...
      if (keep && (pfd.revents & POLLIN) != 0 &&
          !replication.input_backpressured() && !client.read_backpressured) {
      ...
      if (keep && (pfd.revents & POLLOUT) != 0 &&
          has_writable_output(client)) {
        keep = write_client(client, config);
        did_work = true;
      }
  ...
  for (auto& target : targets) {
    if (target.ring_endpoint) {
      ...
      dump_world.remove(*target.ring_endpoint);
...
int Server::run() {
    ...
    if (!kafka) {
      ...
      if (!kafka->catch_up(store_, replay, error, replica.get(),
        ...
        if (!config_.replica_source) {
          ...
          std::cout << "goblin-core: Kafka journal replies use "
                    << (config_.kafka->ack_mode == KafkaAckMode::broker
                            ? "broker acknowledgements"
                            : "local producer queue acceptance")
                    << '\n';
        } else if (!replica) {
  ...
  ReplicationRuntime replication(
      kafka_journal.get(),
      config_.kafka ? config_.kafka->ack_mode : KafkaAckMode::queued,
      config_.replication_buffer_bytes,
      config_.kafka ? config_.kafka->pending_bytes : 0,
      store_.replication_state().offset);
#else
  ...
  const auto poll_kafka_journal = [&]() -> bool {
    ...
    std::string error;
    if (kafka_journal->poll(store_, error)) {
      replication.release_acknowledged();
      return true;
    }
  ...
  const auto network_iteration = [&](int max_timeout_ms) {
...
#endif
    const bool dump_progress = dump_world.pump(store_);
    ...
    if (replica_progress || dump_progress) poll_timeout = 0;
    if (dump_world.needs_reap()) {
      poll_timeout = 0;
    }
#ifdef GOBLIN_HAS_KAFKA
    ...
    const std::size_t dump_fd_count =
        dump_world.notification_fd() >= 0 ? 1U : 0U;
    pollfds.reserve(clients.size() + listener_count + kafka_fd_count +
                    replica_fd_count + dump_fd_count);
    for (const auto& listener : listeners) {
      pollfds.push_back(
    ...
    if (dump_fd_count != 0) {
      pollfds.push_back(pollfd{.fd = dump_world.notification_fd(),
                              .events = POLLIN,
                              .revents = 0});
    }
    for (auto& client_ptr : clients) {
      ...
      pollfds.push_back(pollfd{.fd = client.fd,
                              .events = client_poll_events(
                                  client, replication.input_backpressured()),
                              .revents = 0});
    }
    ...
    const std::size_t dump_event_index =
        listener_count + kafka_fd_count + replica_fd_count;
    if (dump_fd_count != 0 &&
        (pollfds[dump_event_index].revents & (POLLIN | POLLERR | POLLHUP)) !=
            0) {
      (void)dump_world.pump(store_);
    }
    for (std::size_t i = 0; i < clients.size(); ++i) {
      bool keep = !clients[i]->close_requested;
      auto& client = *clients[i];
      const auto revents =
          pollfds[listener_count + kafka_fd_count + replica_fd_count +
                  dump_fd_count + i]
              .revents;
      ...
      if (application_ready && keep && client_read_ready(client, revents) &&
        ...
        keep = read_client(client, store_, pubsub, watches, blocking_lists,
                           ...
                           replication, dump_world, nested_dispatch);
      }

      if (application_ready && keep && has_writable_output(client) &&
    ...
    for (std::size_t i = clients.size(); i > 0; --i) {
      ...
      if (close_client[index] != 0) {
        ...
        dump_world.remove(*clients[index]);
