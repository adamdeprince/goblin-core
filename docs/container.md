...
podman build --format oci \
  --build-arg GOBLIN_CORE_VERSION=0.10.6 \
  --build-arg GOBLIN_CORE_REVISION="$(git rev-parse HEAD)" \
  --file Containerfile \
  --tag localhost/goblin-core:0.10.6 \
  .

podman tag localhost/goblin-core:0.10.6 localhost/goblin-core:latest
```
...
```sh
podman image inspect localhost/goblin-core:0.10.6
podman history localhost/goblin-core:0.10.6
```
