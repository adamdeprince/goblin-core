...
#ifndef _GOBLIN_SBE_TYPE_CXX_H_
#define _GOBLIN_SBE_TYPE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(36);
    ...
    Type(char *buffer, const std::uint64_t bufferLength) :
        Type(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Type(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Type(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Type &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Type &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Type skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Type &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Type &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    Type &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Type &_writer)
{
    Type writer(
        _writer.m_buffer,
        _writer.m_offset,
