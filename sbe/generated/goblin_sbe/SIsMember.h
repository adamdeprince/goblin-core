...
#ifndef _GOBLIN_SBE_SISMEMBER_CXX_H_
#define _GOBLIN_SBE_SISMEMBER_CXX_H_
...
public:
    ...
    SIsMember(char *buffer, const std::uint64_t bufferLength) :
        SIsMember(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SIsMember(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SIsMember(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SIsMember &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SIsMember &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SIsMember skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SIsMember &putKey(const std::string &str)
    {
    ...
    SIsMember &putMember(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SIsMember &_writer)
{
    SIsMember writer(
        _writer.m_buffer,
        _writer.m_offset,
