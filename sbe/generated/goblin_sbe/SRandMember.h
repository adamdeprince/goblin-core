...
#ifndef _GOBLIN_SBE_SRANDMEMBER_CXX_H_
#define _GOBLIN_SBE_SRANDMEMBER_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(109);
    ...
    SRandMember(char *buffer, const std::uint64_t bufferLength) :
        SRandMember(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SRandMember(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SRandMember(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(109);
    }
    ...
    SRandMember &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SRandMember &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SRandMember skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SRandMember &count(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SRandMember &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SRandMember &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    SRandMember &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SRandMember &_writer)
{
    SRandMember writer(
        _writer.m_buffer,
        _writer.m_offset,
