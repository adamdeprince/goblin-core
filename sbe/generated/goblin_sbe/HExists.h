...
#ifndef _GOBLIN_SBE_HEXISTS_CXX_H_
#define _GOBLIN_SBE_HEXISTS_CXX_H_
...
public:
    ...
    HExists(char *buffer, const std::uint64_t bufferLength) :
        HExists(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    HExists(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        HExists(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    HExists &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    HExists &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        HExists skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    HExists &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    HExists &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    HExists &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    HExists &putField(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    HExists &putField(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    HExists &putField(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const HExists &_writer)
{
    HExists writer(
        _writer.m_buffer,
        _writer.m_offset,
