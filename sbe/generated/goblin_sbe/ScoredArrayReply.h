...
#ifndef _GOBLIN_SBE_SCOREDARRAYREPLY_CXX_H_
#define _GOBLIN_SBE_SCOREDARRAYREPLY_CXX_H_
...
public:
    ...
    ScoredArrayReply(char *buffer, const std::uint64_t bufferLength) :
        ScoredArrayReply(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ScoredArrayReply(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ScoredArrayReply(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ScoredArrayReply &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ScoredArrayReply &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ScoredArrayReply &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        ScoredArrayReply skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ScoredArrayReply &_writer)
{
    ScoredArrayReply writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ScoredArrayReply", )";
