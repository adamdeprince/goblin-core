...
#ifndef _GOBLIN_SBE_DOUBLEREPLY_CXX_H_
#define _GOBLIN_SBE_DOUBLEREPLY_CXX_H_
...
public:
    ...
    DoubleReply(char *buffer, const std::uint64_t bufferLength) :
        DoubleReply(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    DoubleReply(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        DoubleReply(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    DoubleReply &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    DoubleReply &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        DoubleReply skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    DoubleReply &value(const double value) SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const DoubleReply &_writer)
{
    DoubleReply writer(
        _writer.m_buffer,
        _writer.m_offset,
