...
#ifndef _GOBLIN_SBE_SADD_CXX_H_
#define _GOBLIN_SBE_SADD_CXX_H_
...
public:
    ...
    SAdd(char *buffer, const std::uint64_t bufferLength) :
        SAdd(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SAdd(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SAdd(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SAdd &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SAdd &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SAdd skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    SAdd &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    SAdd &putKey(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    SAdd &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SAdd &_writer)
{
    SAdd writer(
        _writer.m_buffer,
        _writer.m_offset,
