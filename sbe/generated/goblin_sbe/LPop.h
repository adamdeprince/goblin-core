...
#ifndef _GOBLIN_SBE_LPOP_CXX_H_
#define _GOBLIN_SBE_LPOP_CXX_H_
...
public:
    ...
    LPop(char *buffer, const std::uint64_t bufferLength) :
        LPop(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    LPop(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        LPop(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(87);
    }
    ...
    LPop &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    LPop &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        LPop skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    LPop &count(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    LPop &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    LPop &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    LPop &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const LPop &_writer)
{
    LPop writer(
        _writer.m_buffer,
        _writer.m_offset,
