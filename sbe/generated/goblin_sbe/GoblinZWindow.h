...
#ifndef _GOBLIN_SBE_GOBLINZWINDOW_CXX_H_
#define _GOBLIN_SBE_GOBLINZWINDOW_CXX_H_
...
public:
    ...
    GoblinZWindow(char *buffer, const std::uint64_t bufferLength) :
        GoblinZWindow(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GoblinZWindow(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GoblinZWindow(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GoblinZWindow &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GoblinZWindow &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        GoblinZWindow skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    GoblinZWindow &now(const double value) SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    SBE_NODISCARD static const char *windowMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t windowId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t windowSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool windowInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t windowEncodingOffset() SBE_NOEXCEPT
    {
        return 8;
    }

    static SBE_CONSTEXPR double windowNullValue() SBE_NOEXCEPT
    {
        return SBE_DOUBLE_NAN;
    }

    static SBE_CONSTEXPR double windowMinValue() SBE_NOEXCEPT
    {
        return -1.7976931348623157E308;
    }

    static SBE_CONSTEXPR double windowMaxValue() SBE_NOEXCEPT
    {
        return 1.7976931348623157E308;
    }

    static SBE_CONSTEXPR std::size_t windowEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD double window() const SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
    ...
    GoblinZWindow &window(const double value) SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    GoblinZWindow &limit(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    GoblinZWindow &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    GoblinZWindow &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    GoblinZWindow &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    GoblinZWindow &putMember(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    GoblinZWindow &putMember(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    GoblinZWindow &putMember(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GoblinZWindow &_writer)
{
    GoblinZWindow writer(
        _writer.m_buffer,
        _writer.m_offset,
