...
#ifndef _GOBLIN_SBE_DEL_CXX_H_
#define _GOBLIN_SBE_DEL_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(34);
    ...
    Del(char *buffer, const std::uint64_t bufferLength) :
        Del(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Del(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Del(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Del &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Del &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Del skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Del &_writer)
{
    Del writer(
        _writer.m_buffer,
        _writer.m_offset,
