...
#ifndef _GOBLIN_SBE_SPOP_CXX_H_
#define _GOBLIN_SBE_SPOP_CXX_H_
...
public:
    ...
    SPop(char *buffer, const std::uint64_t bufferLength) :
        SPop(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SPop(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SPop(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SPop &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SPop &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SPop skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SPop &count(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SPop &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SPop &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    SPop &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SPop &_writer)
{
    SPop writer(
        _writer.m_buffer,
        _writer.m_offset,
