...
#ifndef _GOBLIN_SBE_GOBLININCREX_CXX_H_
#define _GOBLIN_SBE_GOBLININCREX_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(70);
    ...
    GoblinIncrEx(char *buffer, const std::uint64_t bufferLength) :
        GoblinIncrEx(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GoblinIncrEx(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GoblinIncrEx(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GoblinIncrEx &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GoblinIncrEx &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        GoblinIncrEx skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    GoblinIncrEx &seconds(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    GoblinIncrEx &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    GoblinIncrEx &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    GoblinIncrEx &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GoblinIncrEx &_writer)
{
    GoblinIncrEx writer(
        _writer.m_buffer,
        _writer.m_offset,
