...
#ifndef _GOBLIN_SBE_PEXPIRE_CXX_H_
#define _GOBLIN_SBE_PEXPIRE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(38);
    ...
    PExpire(char *buffer, const std::uint64_t bufferLength) :
        PExpire(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    PExpire(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        PExpire(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    PExpire &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PExpire &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        PExpire skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    PExpire &amount(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    PExpire &flags(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    PExpire &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    PExpire &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    PExpire &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PExpire &_writer)
{
    PExpire writer(
        _writer.m_buffer,
        _writer.m_offset,
