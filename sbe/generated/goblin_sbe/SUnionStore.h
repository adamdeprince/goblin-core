...
#ifndef _GOBLIN_SBE_SUNIONSTORE_CXX_H_
#define _GOBLIN_SBE_SUNIONSTORE_CXX_H_
...
public:
    ...
    SUnionStore(char *buffer, const std::uint64_t bufferLength) :
        SUnionStore(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SUnionStore(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SUnionStore(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(115);
    }
    ...
    SUnionStore &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SUnionStore &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SUnionStore skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        std::uint64_t skipKey()
        {
            ...
            std::uint32_t lengthKeyValue;
            std::memcpy(&lengthKeyValue, m_buffer + lengthPosition, sizeof(std::uint32_t));
            std::uint64_t dataLength = SBE_LITTLE_ENDIAN_ENCODE_32(lengthKeyValue);
            sbePosition(lengthPosition + lengthOfLengthKey + dataLength);
            return dataLength;
        }
        ...
        {
            std::uint32_t lengthKeyValue;
            std::memcpy(&lengthKeyValue, m_buffer + sbePosition(), sizeof(std::uint32_t));
            const char *keyPtr = m_buffer + sbePosition() + 4;
            sbePosition(sbePosition() + 4 + SBE_LITTLE_ENDIAN_ENCODE_32(lengthKeyValue));
            return keyPtr;
        }
        ...
        std::uint64_t getDestination(char *dst, const std::uint64_t length)
        {
            std::uint64_t lengthOfLengthKey = 4;
            std::uint64_t lengthPosition = sbePosition();
            sbePosition(lengthPosition + lengthOfLengthKey);
            std::uint32_t lengthKeyValue;
            std::memcpy(&lengthKeyValue, m_buffer + lengthPosition, sizeof(std::uint32_t));
            std::uint64_t dataLength = SBE_LITTLE_ENDIAN_ENCODE_32(lengthKeyValue);
        ...
        Keys &putDestination(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthKey = 4;
            std::uint64_t lengthPosition = sbePosition();
            std::uint32_t lengthKeyValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
            sbePosition(lengthPosition + lengthOfLengthKey);
            std::memcpy(m_buffer + lengthPosition, &lengthKeyValue, sizeof(std::uint32_t));
...
public:
    ...
    SUnionStore &putDestination(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthKey = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthKeyValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    SUnionStore &putDestination(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    SUnionStore &putDestination(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SUnionStore &_writer)
{
    SUnionStore writer(
        _writer.m_buffer,
        _writer.m_offset,
