...
#ifndef _GOBLIN_SBE_VARDATA_CXX_H_
#define _GOBLIN_SBE_VARDATA_CXX_H_
...
public:
    ...
    VarData(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        VarData(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    VarData(
        char *buffer,
        const std::uint64_t bufferLength) :
        VarData(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    VarData &length(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, VarData &writer)
{
    builder << '{';
    builder << R"("length": )";
