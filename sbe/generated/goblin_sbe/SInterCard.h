...
#ifndef _GOBLIN_SBE_SINTERCARD_CXX_H_
#define _GOBLIN_SBE_SINTERCARD_CXX_H_
...
public:
    ...
    SInterCard(char *buffer, const std::uint64_t bufferLength) :
        SInterCard(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SInterCard(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SInterCard(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(117);
    }
    ...
    SInterCard &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SInterCard &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SInterCard skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SInterCard &_writer)
{
    SInterCard writer(
        _writer.m_buffer,
        _writer.m_offset,
...
SBE_NODISCARD static std::size_t computeLength(const std::vector<std::tuple<std::size_t>> &keysItemLengths = {})
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
