...
#ifndef _GOBLIN_SBE_ARRAYREPLY_CXX_H_
#define _GOBLIN_SBE_ARRAYREPLY_CXX_H_
...
public:
    ...
    ArrayReply(char *buffer, const std::uint64_t bufferLength) :
        ArrayReply(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ArrayReply(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ArrayReply(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ArrayReply &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ArrayReply &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ArrayReply skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ArrayReply &_writer)
{
    ArrayReply writer(
        _writer.m_buffer,
        _writer.m_offset,
