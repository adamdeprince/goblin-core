...
#ifndef _GOBLIN_SBE_HGETALL_CXX_H_
#define _GOBLIN_SBE_HGETALL_CXX_H_
...
public:
    ...
    HGetAll(char *buffer, const std::uint64_t bufferLength) :
        HGetAll(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    HGetAll(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        HGetAll(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    HGetAll &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    HGetAll &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        HGetAll skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    HGetAll &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    HGetAll &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    HGetAll &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const HGetAll &_writer)
{
    HGetAll writer(
        _writer.m_buffer,
        _writer.m_offset,
