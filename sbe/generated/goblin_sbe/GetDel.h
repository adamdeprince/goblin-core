...
#ifndef _GOBLIN_SBE_GETDEL_CXX_H_
#define _GOBLIN_SBE_GETDEL_CXX_H_
...
public:
    ...
    GetDel(char *buffer, const std::uint64_t bufferLength) :
        GetDel(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GetDel(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GetDel(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GetDel &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GetDel &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        GetDel skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    GetDel &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    GetDel &putKey(const std::string &str)
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GetDel &_writer)
{
    GetDel writer(
        _writer.m_buffer,
        _writer.m_offset,
