...
#ifndef _GOBLIN_SBE_UNSUBSCRIBE_CXX_H_
#define _GOBLIN_SBE_UNSUBSCRIBE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(97);
    ...
    Unsubscribe(char *buffer, const std::uint64_t bufferLength) :
        Unsubscribe(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Unsubscribe(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Unsubscribe(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Unsubscribe &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Unsubscribe &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Unsubscribe skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        Channels &putChannel(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthField = 4;
            std::uint64_t lengthPosition = sbePosition();
        ...
        Channels &putChannel(const std::string &str)
        {
        ...
        #if __cplusplus >= 201703L
        Channels &putChannel(const std::string_view str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        friend std::basic_ostream<CharT, Traits> & operator << (
            std::basic_ostream<CharT, Traits> &builder, Channels &writer)
        {
            builder << '{';
            builder << R"("channel": )";
            builder << '"' <<
...
public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t channelsId() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD inline Channels &channels()
    {
        m_channels.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_channels;
    }

    Channels &channelsCount(const std::uint16_t count)
    {
        m_channels.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_channels;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t channelsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool channelsInActingVersion() const SBE_NOEXCEPT
    {
        return true;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Unsubscribe &_writer)
{
    Unsubscribe writer(
        _writer.m_buffer,
        _writer.m_offset,
...
void skip()
{
    auto &channelsGroup { channels() };
    while (channelsGroup.hasNext())
    {
        channelsGroup.next().skip();
    }
}
...
SBE_NODISCARD static std::size_t computeLength(const std::vector<std::tuple<std::size_t>> &channelsItemLengths = {})
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += Channels::sbeHeaderSize();
    if (channelsItemLengths.size() > 65534LL)
    {
        throw std::runtime_error("channelsItemLengths.size() outside of allowed range [E110]");
    }

    for (const auto &e: channelsItemLengths)
    {
        #if __cplusplus >= 201703L
        length += std::apply(Channels::computeLength, e);
        #else
        length += Channels::computeLength(std::get<0>(e));
        #endif
    }
