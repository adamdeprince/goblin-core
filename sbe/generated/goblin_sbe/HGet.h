...
#ifndef _GOBLIN_SBE_HGET_CXX_H_
#define _GOBLIN_SBE_HGET_CXX_H_
...
public:
    ...
    HGet(char *buffer, const std::uint64_t bufferLength) :
        HGet(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    HGet(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        HGet(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    HGet &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    HGet &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        HGet skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    HGet &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    HGet &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    HGet &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    HGet &putField(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    HGet &putField(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    HGet &putField(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const HGet &_writer)
{
    HGet writer(
        _writer.m_buffer,
        _writer.m_offset,
