...
#ifndef _GOBLIN_SBE_SDIFF_CXX_H_
#define _GOBLIN_SBE_SDIFF_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(113);
    ...
    SDiff(char *buffer, const std::uint64_t bufferLength) :
        SDiff(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SDiff(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SDiff(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(113);
    }
    ...
    SDiff &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SDiff &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SDiff skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SDiff &_writer)
{
    SDiff writer(
        _writer.m_buffer,
        _writer.m_offset,
