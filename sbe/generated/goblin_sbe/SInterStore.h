...
#ifndef _GOBLIN_SBE_SINTERSTORE_CXX_H_
#define _GOBLIN_SBE_SINTERSTORE_CXX_H_
...
public:
    ...
    SInterStore(char *buffer, const std::uint64_t bufferLength) :
        SInterStore(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SInterStore(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SInterStore(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(114);
    }
    ...
    SInterStore &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SInterStore &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SInterStore skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    SInterStore &putDestination(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthKey = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthKeyValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    SInterStore &putDestination(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    SInterStore &putDestination(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SInterStore &_writer)
{
    SInterStore writer(
        _writer.m_buffer,
        _writer.m_offset,
