...
#ifndef _GOBLIN_SBE_ZREM_CXX_H_
#define _GOBLIN_SBE_ZREM_CXX_H_
...
public:
    ...
    ZRem(char *buffer, const std::uint64_t bufferLength) :
        ZRem(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ZRem(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ZRem(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ZRem &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ZRem &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ZRem skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    ZRem &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    ZRem &putKey(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    ZRem &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ZRem &_writer)
{
    ZRem writer(
        _writer.m_buffer,
        _writer.m_offset,
