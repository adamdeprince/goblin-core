...
#ifndef _GOBLIN_SBE_SETNX_CXX_H_
#define _GOBLIN_SBE_SETNX_CXX_H_
...
public:
    ...
    SetNx(char *buffer, const std::uint64_t bufferLength) :
        SetNx(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SetNx(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SetNx(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SetNx &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SetNx &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SetNx skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SetNx &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SetNx &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    SetNx &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    SetNx &putValue(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    SetNx &putValue(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    SetNx &putValue(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SetNx &_writer)
{
    SetNx writer(
        _writer.m_buffer,
        _writer.m_offset,
