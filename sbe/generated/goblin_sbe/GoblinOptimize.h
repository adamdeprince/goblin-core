...
#ifndef _GOBLIN_SBE_GOBLINOPTIMIZE_CXX_H_
#define _GOBLIN_SBE_GOBLINOPTIMIZE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(77);
    ...
    GoblinOptimize(char *buffer, const std::uint64_t bufferLength) :
        GoblinOptimize(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GoblinOptimize(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GoblinOptimize(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GoblinOptimize &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GoblinOptimize &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        GoblinOptimize skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SBE_NODISCARD static const char *densityMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
    ...
    static SBE_CONSTEXPR std::uint16_t densityId() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t densitySinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool densityInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t densityEncodingOffset() SBE_NOEXCEPT
    {
        return 0;
    }

    static SBE_CONSTEXPR double densityNullValue() SBE_NOEXCEPT
    {
        return SBE_DOUBLE_NAN;
    }

    static SBE_CONSTEXPR double densityMinValue() SBE_NOEXCEPT
    {
        return -1.7976931348623157E308;
    }

    static SBE_CONSTEXPR double densityMaxValue() SBE_NOEXCEPT
    {
        return 1.7976931348623157E308;
    }

    static SBE_CONSTEXPR std::size_t densityEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD double density() const SBE_NOEXCEPT
    {
    ...
    GoblinOptimize &density(const double value) SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    GoblinOptimize &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    GoblinOptimize &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    GoblinOptimize &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GoblinOptimize &_writer)
{
    GoblinOptimize writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "GoblinOptimize", )";
    builder << R"("sbeTemplateId": )";
    builder << writer.sbeTemplateId();
    builder << ", ";

    builder << R"("density": )";
    builder << +writer.density();
