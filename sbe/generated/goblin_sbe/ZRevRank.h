...
#ifndef _GOBLIN_SBE_ZREVRANK_CXX_H_
#define _GOBLIN_SBE_ZREVRANK_CXX_H_
...
public:
    ...
    ZRevRank(char *buffer, const std::uint64_t bufferLength) :
        ZRevRank(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ZRevRank(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ZRevRank(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ZRevRank &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ZRevRank &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ZRevRank skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ZRevRank &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ZRevRank &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    ZRevRank &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    ZRevRank &putMember(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    ZRevRank &putMember(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    ZRevRank &putMember(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ZRevRank &_writer)
{
    ZRevRank writer(
        _writer.m_buffer,
        _writer.m_offset,
