...
#ifndef _GOBLIN_SBE_SINTER_CXX_H_
#define _GOBLIN_SBE_SINTER_CXX_H_
...
public:
    ...
    SInter(char *buffer, const std::uint64_t bufferLength) :
        SInter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SInter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SInter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SInter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SInter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SInter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SInter &_writer)
{
    SInter writer(
        _writer.m_buffer,
        _writer.m_offset,
