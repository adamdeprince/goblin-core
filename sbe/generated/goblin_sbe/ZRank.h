...
#ifndef _GOBLIN_SBE_ZRANK_CXX_H_
#define _GOBLIN_SBE_ZRANK_CXX_H_
...
public:
    ...
    ZRank(char *buffer, const std::uint64_t bufferLength) :
        ZRank(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ZRank(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ZRank(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ZRank &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ZRank &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ZRank skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ZRank &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ZRank &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    ZRank &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    ZRank &putMember(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    ZRank &putMember(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    ZRank &putMember(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ZRank &_writer)
{
    ZRank writer(
        _writer.m_buffer,
        _writer.m_offset,
