...
#ifndef _GOBLIN_SBE_HINCRBY_CXX_H_
#define _GOBLIN_SBE_HINCRBY_CXX_H_
...
public:
    ...
    HIncrBy(char *buffer, const std::uint64_t bufferLength) :
        HIncrBy(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    HIncrBy(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        HIncrBy(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    HIncrBy &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    HIncrBy &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        HIncrBy skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    static SBE_CONSTEXPR std::int64_t deltaNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t deltaMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t deltaMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }
    ...
    SBE_NODISCARD std::int64_t delta() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    HIncrBy &delta(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    HIncrBy &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    HIncrBy &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    HIncrBy &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    HIncrBy &putField(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    HIncrBy &putField(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    HIncrBy &putField(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const HIncrBy &_writer)
{
    HIncrBy writer(
        _writer.m_buffer,
        _writer.m_offset,
