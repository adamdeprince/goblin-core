...
#ifndef _GOBLIN_SBE_SMOVE_CXX_H_
#define _GOBLIN_SBE_SMOVE_CXX_H_
...
public:
    ...
    SMove(char *buffer, const std::uint64_t bufferLength) :
        SMove(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SMove(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SMove(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(110);
    }
    ...
    SMove &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SMove &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SMove skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SBE_NODISCARD static const char *sourceMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD std::uint32_t sourceLength() const
    {
        std::uint32_t length;
    ...
    std::uint64_t getSource(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SMove &putSource(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getSourceAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getSourceAsString();
    ...
    std::string_view getSourceAsStringView()
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    SMove &putSource(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putSource(str.data(), static_cast<std::uint32_t>(str.length()));
    }

    #ifdef SBE_USE_STRING_VIEW
    SMove &putSource(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    SMove &putDestination(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    SMove &putDestination(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    SMove &putDestination(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    SMove &putMember(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    SMove &putMember(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #ifdef SBE_USE_STRING_VIEW
    SMove &putMember(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SMove &_writer)
{
    SMove writer(
        _writer.m_buffer,
        _writer.m_offset,
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    if (sourceLength > 1073741824LL)
    {
        throw std::runtime_error("sourceLength too long for length type [E109]");
    }
    length += sourceLength;
