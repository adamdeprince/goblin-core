...
#ifndef _GOBLIN_SBE_LRANGE_CXX_H_
#define _GOBLIN_SBE_LRANGE_CXX_H_
...
public:
    ...
    LRange(char *buffer, const std::uint64_t bufferLength) :
        LRange(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    LRange(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        LRange(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    LRange &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    LRange &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        LRange skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    LRange &start(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    LRange &stop(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    LRange &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    LRange &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    LRange &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const LRange &_writer)
{
    LRange writer(
        _writer.m_buffer,
        _writer.m_offset,
