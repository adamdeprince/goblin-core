...
#ifndef _GOBLIN_SBE_MSET_CXX_H_
#define _GOBLIN_SBE_MSET_CXX_H_
...
public:
    ...
    MSet(char *buffer, const std::uint64_t bufferLength) :
        MSet(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MSet(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MSet(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MSet &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MSet &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MSet skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        Pairs &putKey(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthField = 4;
            std::uint64_t lengthPosition = sbePosition();
        ...
        Pairs &putKey(const std::string &str)
        {
        ...
        #ifdef SBE_USE_STRING_VIEW
        Pairs &putKey(const std::string_view str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        Pairs &putValue(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthField = 4;
            std::uint64_t lengthPosition = sbePosition();
            std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
        ...
        Pairs &putValue(const std::string &str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        #ifdef SBE_USE_STRING_VIEW
        Pairs &putValue(const std::string_view str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        friend std::basic_ostream<CharT, Traits> & operator << (
            std::basic_ostream<CharT, Traits> &builder, Pairs &writer)
        {
            builder << '{';
            builder << R"("key": )";
            builder << '"' <<
...
public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t pairsId() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD inline Pairs &pairs()
    {
        m_pairs.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_pairs;
    }

    Pairs &pairsCount(const std::uint16_t count)
    {
        m_pairs.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_pairs;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t pairsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool pairsInActingVersion() const SBE_NOEXCEPT
    {
        return true;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MSet &_writer)
{
    MSet writer(
        _writer.m_buffer,
        _writer.m_offset,
...
void skip()
{
    auto &pairsGroup { pairs() };
    while (pairsGroup.hasNext())
    {
        pairsGroup.next().skip();
    }
}
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += Pairs::sbeHeaderSize();
    if (pairsItemLengths.size() > 65534LL)
    {
        throw std::runtime_error("pairsItemLengths.size() outside of allowed range [E110]");
    }

    for (const auto &e: pairsItemLengths)
    {
        #if __cplusplus >= 201703L
        length += std::apply(Pairs::computeLength, e);
        #else
        length += Pairs::computeLength(std::get<0>(e), std::get<1>(e));
        #endif
    }
