...
#ifndef _GOBLIN_SBE_GOBLININCRBOUND_CXX_H_
#define _GOBLIN_SBE_GOBLININCRBOUND_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(72);
    ...
    GoblinIncrBound(char *buffer, const std::uint64_t bufferLength) :
        GoblinIncrBound(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    GoblinIncrBound(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        GoblinIncrBound(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    GoblinIncrBound &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    GoblinIncrBound &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        GoblinIncrBound skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    GoblinIncrBound &delta(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    static SBE_CONSTEXPR std::int64_t maxNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t maxMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t maxMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }
    ...
    SBE_NODISCARD std::int64_t max() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    GoblinIncrBound &max(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    GoblinIncrBound &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    GoblinIncrBound &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    GoblinIncrBound &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const GoblinIncrBound &_writer)
{
    GoblinIncrBound writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "GoblinIncrBound", )";
