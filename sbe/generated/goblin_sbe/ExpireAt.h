...
#ifndef _GOBLIN_SBE_EXPIREAT_CXX_H_
#define _GOBLIN_SBE_EXPIREAT_CXX_H_
...
public:
    ...
    ExpireAt(char *buffer, const std::uint64_t bufferLength) :
        ExpireAt(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ExpireAt(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ExpireAt(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ExpireAt &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ExpireAt &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ExpireAt skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ExpireAt &amount(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ExpireAt &flags(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    ExpireAt &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ExpireAt &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    ExpireAt &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ExpireAt &_writer)
{
    ExpireAt writer(
        _writer.m_buffer,
        _writer.m_offset,
