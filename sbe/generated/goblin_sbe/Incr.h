...
#ifndef _GOBLIN_SBE_INCR_CXX_H_
#define _GOBLIN_SBE_INCR_CXX_H_
...
public:
    ...
    Incr(char *buffer, const std::uint64_t bufferLength) :
        Incr(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Incr(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Incr(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Incr &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Incr &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Incr skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Incr &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Incr &putKey(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    Incr &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Incr &_writer)
{
    Incr writer(
        _writer.m_buffer,
        _writer.m_offset,
