...
#ifndef _GOBLIN_SBE_ZREMRANGEBYSCORE_CXX_H_
#define _GOBLIN_SBE_ZREMRANGEBYSCORE_CXX_H_
...
public:
    ...
    ZRemRangeByScore(char *buffer, const std::uint64_t bufferLength) :
        ZRemRangeByScore(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ZRemRangeByScore(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ZRemRangeByScore(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ZRemRangeByScore &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ZRemRangeByScore &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ZRemRangeByScore &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        ZRemRangeByScore skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ZRemRangeByScore &min(const double value) SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    ZRemRangeByScore &minExclusive(const std::uint8_t value) SBE_NOEXCEPT
    {
    ...
    ZRemRangeByScore &max(const double value) SBE_NOEXCEPT
    {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    ZRemRangeByScore &maxExclusive(const std::uint8_t value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    ZRemRangeByScore &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ZRemRangeByScore &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    ZRemRangeByScore &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ZRemRangeByScore &_writer)
{
    ZRemRangeByScore writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ZRemRangeByScore", )";
