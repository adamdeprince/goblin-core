...
#ifndef _GOBLIN_SBE_MAPREPLY_CXX_H_
#define _GOBLIN_SBE_MAPREPLY_CXX_H_
...
public:
    ...
    MapReply(char *buffer, const std::uint64_t bufferLength) :
        MapReply(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MapReply(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MapReply(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MapReply &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MapReply &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MapReply skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        Entries &putKey(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthField = 4;
            std::uint64_t lengthPosition = sbePosition();
        ...
        Entries &putKey(const std::string &str)
        {
        ...
        #ifdef SBE_USE_STRING_VIEW
        Entries &putKey(const std::string_view str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MapReply &_writer)
{
    MapReply writer(
        _writer.m_buffer,
        _writer.m_offset,
