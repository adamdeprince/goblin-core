...
#ifndef _GOBLIN_SBE_HKEYS_CXX_H_
#define _GOBLIN_SBE_HKEYS_CXX_H_
...
public:
    ...
    HKeys(char *buffer, const std::uint64_t bufferLength) :
        HKeys(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    HKeys(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        HKeys(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    HKeys &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    HKeys &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        HKeys skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    HKeys &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    HKeys &putKey(const std::string &str)
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    HKeys &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const HKeys &_writer)
{
    HKeys writer(
        _writer.m_buffer,
        _writer.m_offset,
