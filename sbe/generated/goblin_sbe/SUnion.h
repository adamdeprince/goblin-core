...
#ifndef _GOBLIN_SBE_SUNION_CXX_H_
#define _GOBLIN_SBE_SUNION_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(112);
    ...
    SUnion(char *buffer, const std::uint64_t bufferLength) :
        SUnion(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SUnion(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SUnion(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SUnion &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SUnion &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SUnion skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SUnion &_writer)
{
    SUnion writer(
        _writer.m_buffer,
        _writer.m_offset,
