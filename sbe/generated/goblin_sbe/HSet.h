...
#ifndef _GOBLIN_SBE_HSET_CXX_H_
#define _GOBLIN_SBE_HSET_CXX_H_
...
public:
    ...
    HSet(char *buffer, const std::uint64_t bufferLength) :
        HSet(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    HSet(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        HSet(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    HSet &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    HSet &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        HSet skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        Entries &putField(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthField = 4;
            std::uint64_t lengthPosition = sbePosition();
        ...
        Entries &putField(const std::string &str)
        {
        ...
        #if __cplusplus >= 201703L
        Entries &putField(const std::string_view str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        Entries &putValue(const char *src, const std::uint32_t length)
        {
            std::uint64_t lengthOfLengthField = 4;
            std::uint64_t lengthPosition = sbePosition();
            std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
        ...
        Entries &putValue(const std::string &str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        #if __cplusplus >= 201703L
        Entries &putValue(const std::string_view str)
        {
            if (str.length() > 1073741824)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
...
public:
    ...
    HSet &putKey(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
        std::uint32_t lengthFieldValue = SBE_LITTLE_ENDIAN_ENCODE_32(length);
    ...
    HSet &putKey(const std::string &str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    HSet &putKey(const std::string_view str)
    {
        if (str.length() > 1073741824)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const HSet &_writer)
{
    HSet writer(
        _writer.m_buffer,
        _writer.m_offset,
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    if (entriesItemLengths.size() > 65534LL)
    {
        throw std::runtime_error("entriesItemLengths.size() outside of allowed range [E110]");
    }

    for (const auto &e: entriesItemLengths)
    {
        #if __cplusplus >= 201703L
        length += std::apply(Entries::computeLength, e);
        #else
        length += Entries::computeLength(std::get<0>(e), std::get<1>(e));
        #endif
    }
