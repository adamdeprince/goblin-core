...
#ifndef _GOBLIN_SBE_MGET_CXX_H_
#define _GOBLIN_SBE_MGET_CXX_H_
...
public:
    ...
    MGet(char *buffer, const std::uint64_t bufferLength) :
        MGet(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MGet(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MGet(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MGet &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MGet &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MGet skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MGet &_writer)
{
    MGet writer(
        _writer.m_buffer,
        _writer.m_offset,
