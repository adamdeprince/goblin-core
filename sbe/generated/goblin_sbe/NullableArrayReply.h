...
#ifndef _GOBLIN_SBE_NULLABLEARRAYREPLY_CXX_H_
#define _GOBLIN_SBE_NULLABLEARRAYREPLY_CXX_H_
...
public:
    ...
    NullableArrayReply(char *buffer, const std::uint64_t bufferLength) :
        NullableArrayReply(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    NullableArrayReply(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        NullableArrayReply(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    NullableArrayReply &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    NullableArrayReply &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    NullableArrayReply &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        NullableArrayReply skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        SBE_NODISCARD static const char *presentMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
        {
        ...
        static SBE_CONSTEXPR std::uint16_t presentId() SBE_NOEXCEPT
        {
            return 1;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t presentSinceVersion() SBE_NOEXCEPT
        {
            return 0;
        }

        SBE_NODISCARD bool presentInActingVersion() SBE_NOEXCEPT
        {
            return true;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::size_t presentEncodingOffset() SBE_NOEXCEPT
        {
            return 0;
        }

        static SBE_CONSTEXPR std::uint8_t presentNullValue() SBE_NOEXCEPT
        {
            return SBE_NULLVALUE_UINT8;
        }

        static SBE_CONSTEXPR std::uint8_t presentMinValue() SBE_NOEXCEPT
        {
            return static_cast<std::uint8_t>(0);
        }

        static SBE_CONSTEXPR std::uint8_t presentMaxValue() SBE_NOEXCEPT
        {
            return static_cast<std::uint8_t>(254);
        }

        static SBE_CONSTEXPR std::size_t presentEncodingLength() SBE_NOEXCEPT
        {
            return 1;
        }

        SBE_NODISCARD std::uint8_t present() const SBE_NOEXCEPT
        {
            std::uint8_t val;
        ...
        Items &present(const std::uint8_t value) SBE_NOEXCEPT
        {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const NullableArrayReply &_writer)
{
    NullableArrayReply writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "NullableArrayReply", )";
