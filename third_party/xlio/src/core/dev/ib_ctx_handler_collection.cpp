...
bool has_ethernet_port(struct ibv_device *device)
{
...
void ib_ctx_handler_collection::update_tbl(const char *ifa_name)
{
    ...
    for (i = 0; i < num_devices; i++) {
        ...
        // XLIO accelerates Ethernet. Mixed-fabric hosts can also expose
        // InfiniBand-only verbs devices that DPCP cannot open.
        if (!has_ethernet_port(dev_list[i])) {
            ibchc_logdbg("Skipping non-Ethernet IB device %s", dev_list[i]->name);
            continue;
        }
